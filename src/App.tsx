...
const KYSYMYKSET = [
  {
    ...
    topic: 'Solubiologia',
    question:
      'Mikä seuraavista väittämistä koskien solukalvon rakennetta ja toimintaa on TOSI?',
    options: [
  ...
  },
  {
    ...
    question:
      'Mikä on 0,01 M HCl-liuoksen pH? Oletetaan, että suolahappo protolysoituu täydellisesti.',
    options: ['1', '2', '3', '0.01'],
    correctAnswer: 1,
    explanation:
      'HCl on vahva happo, joten se protolysoituu täydellisesti: [H3O+] = 0,01 mol/l. pH = -log(0,01) = 2.',
  },
  {
    id: 3,
    subject: 'Fysiikka',
    topic: 'Mekaniikka',
    question:
      'Kappale (m = 2,0 kg) putoaa vapaasti 5,0 metrin korkeudesta. Kuinka suuri on sen kineettinen energia juuri ennen maahan osumista? (g ≈ 9,81 m/s²)',
    options: ['49 J', '98 J', '10 J', '196 J'],
    correctAnswer: 1,
    explanation:
      'Mekaanisen energian säilymislain mukaan potentiaalienergia muuttuu kineettiseksi energiaksi. Ep = mgh = 2,0 kg * 9,81 m/s² * 5,0 m = 98,1 J ≈ 98 J.',
  },
  {
    id: 4,
    subject: 'Biologia',
    ...
    options: ['Sappirakko', 'Maksa', 'Haima', 'Ohutsuoli'],
    correctAnswer: 1,
    explanation:
      'Sappineste muodostuu maksassa ja vain varastoituu sekä väkevöityy sappirakossa. Tämä on yleinen kompa pääsykokeissa.',
  },
];
...
const FLASHCARDS = [
  {
    id: 1,
    front: 'Mitoosin vaiheet järjestyksessä?',
    back: 'Profaasi, prometafaasi, metafaasi, anafaasi, telofaasi.',
  },
  {
    id: 2,
    front: 'Ohmin laki (kaava)?',
    back: 'U = RI (Jännite = Resistanssi × Virta)',
  },
  {
    id: 3,
    front: 'Mitä tarkoittaa isotooppi?',
    back: 'Saman alkuaineen atomi, jolla on eri määrä neutroneja ytimessä (eri massaluku).',
  },
  {
    id: 4,
    front:
      'Verenkierron iso verenkierto (systeeminen) alkaa mistä sydämen osasta?',
    back: 'Vasen kammio (aortan kautta koko kehoon).',
  },
];
...
const callGeminiAPI = async (prompt, isJson = false) => {
  ...
  // Kokeillaan useampaa mallia, jotta löydetään API-avaimelle varmasti sallittu versio
  const modelsToTry = [
    'gemini-2.5-flash',
    'gemini-2.0-flash',
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          continue; // Kokeillaan heti seuraavaa mallia
        }
        ...
        throw new Error(errMsg); // Muut virheet (esim 403) heitetään eteenpäin ja lopetetaan
      }
      ...
      return text; // Onnistui! Palautetaan teksti ja katkaistaan silmukka
    } catch (error) {
      if (error.message.includes('404')) {
        continue; // 404 tapauksessa jatketaan silmukkaa
      }
      console.error('Yksityiskohtainen virhe API-kutsussa:', error);
      if (error.message.includes('Failed to fetch')) {
        throw new Error(
          'Nettiyhteys katkesi tai selaimen mainostenestäjä estää yhteyden Googlen palvelimelle.'
        );
      }
      throw error; // Muut virheet kaatavat suorituksen
    }
  }

  // Jos päästään tänne asti, mikään malli ei toiminut
      ...
      <div className="flex justify-between h-16">
        ...
        >
          ...
          <span className="font-bold text-xl tracking-tight">
            Lääkis<span className="text-emerald-400">Treeni</span>
          </span>
        </div>
...
const Dashboard = ({ setCurrentView, stats }) => {
  ...
  const testApiConnection = async () => {
    ...
    try {
      ...
      setTestStatus(
        `Yhteys toimii täydellisesti! Tekoäly vastasi: "${response.trim()}"`
      );
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-500">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2 text-lg">
          Valintakoe B (Lääketiede, hammaslääketiede, eläinlääketiede) lähestyy.
      ...
      <div className="bg-slate-100 border border-slate-300 p-5 mb-8 rounded-xl">
        <h3 className="font-bold text-slate-800 mb-2 flex items-center">
          <RefreshCw className="h-5 w-5 mr-2" /> Vianmääritys: Testaa tekoälyn
          yhteys
        </h3>
        <p className="text-sm text-slate-600 mb-4">
          Jos uudet muistikortit eivät toimi, paina tästä nähdäksesi tarkan
          virheen.
        </p>
          ...
          >
            <div className="text-left">
              ...
              <p className="text-emerald-100 mt-1">
                Sekoitus Valintakoe B -tyyppisiä tehtäviä. Välitön palaute.
              </p>
            </div>
          ...
          >
            <div className="text-left">
              ...
              <p className="text-slate-500 mt-1">
                Nopea kaavojen ja termien kertaus. Spaced repetition.
              </p>
            </div>
...
const Quiz = ({ questions, onComplete }) => {
  ...
  const handleAskAi = async () => {
    ...
    setAiExplanation('');
    try {
      const prompt = `Olen lääketieteen pääsykokeisiin valmistautuva opiskelija. Kysymys oli: "${
        question.question
      }". Oikea vastaus on "${
        question.options[question.correctAnswer]
      }". Minun vastaukseni oli "${
        question.options[selectedAnswer]
      }". Selitä lyhyesti (max 3-4 virkettä), kannustavasti ja selkeästi suomeksi miksi oikea vastaus on oikein, ja jos vastasin väärin, miksi vastaukseni oli virheellinen. Korosta lääketieteellistä/luonnontieteellistä logiikkaa.`;
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="space-y-3">
          {question.options.map((option, idx) => (
            <button
              ...
              disabled={isAnswered}
              className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-center justify-between ${getOptionStyles(
                idx
              )}`}
            >
              <span>{option}</span>
      ...
      {isAnswered && (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          {aiExplanation && (
            <div
              className={`p-6 rounded-xl mb-6 border animate-in fade-in duration-500 ${
                aiExplanation.includes('VIRHE:')
                  ? 'bg-red-50 border-red-200'
...
const Flashcards = ({ cards, setCards, user }) => {
  ...
  if (!cards || cards.length === 0) {
    return (
      <div className="p-8 text-center text-slate-500">Ladataan kortteja...</div>
    );
  }
  ...
  const handleGenerateCards = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          'Kortteja ei saatu tallennettua tietokantaan, lisätään paikallisesti.',
          dbErr
        );
      }

      const localCards = newCardsData.map((c, i) => ({
        id: Date.now() + i,
        front: c.front,
        back: c.back,
      }));
      setCards((prev) => [...localCards, ...prev]);
    ...
    <div className="max-w-2xl mx-auto px-4 py-12 flex flex-col items-center">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500">
          Mieti vastaus mielessäsi ennen kuin käännät kortin.
        </p>
        <p className="text-sm font-medium text-slate-400 mt-2">
          Kortti {currentCard + 1} / {cards.length}
        </p>
      </div>
      ...
      {isFlipped && (
        <div className="w-full flex space-x-4 animate-in fade-in duration-300">
          <button
            ...
            className="flex-1 bg-red-100 hover:bg-red-200 text-red-800 font-semibold py-3 rounded-lg transition-colors"
          >
            Vaikea (Kertaa pian)
          </button>
          <button
            onClick={handleNextCard}
            className="flex-1 bg-blue-100 hover:bg-blue-200 text-blue-800 font-semibold py-3 rounded-lg transition-colors"
          >
            Hyvä
          </button>
          <button
            onClick={handleNextCard}
            className="flex-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-semibold py-3 rounded-lg transition-colors"
          >
            Helppo
          </button>
        </div>
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        setUser({ uid: 'paikallinen-testi-käyttäjä-123' });
      }
    };
    initAuth();
      ...
      async (snapshot) => {
        if (snapshot.empty) {
          try {
            ...
            console.warn(
              'Tietokantaan kirjoittaminen estetty. Käytetään varakysymyksiä.',
              e
            );
      ...
      async (snapshot) => {
        if (snapshot.empty) {
          try {
            ...
            console.warn(
              'Tietokantaan kirjoittaminen estetty. Käytetään varakortteja.',
              e
            );
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <main>
        {isLoadingDb ? (
          ...
          <>
            {currentView === 'dashboard' && (
              <Dashboard setCurrentView={setCurrentView} stats={stats} />
            )}
            {currentView === 'quiz' && (
